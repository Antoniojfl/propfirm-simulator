...
export class MonteCarloEngine {
  ...
  private resampleTrades(rng: Rng): RawTrade[] {
    ...
    const n = this.rawTrades.length;
    if (n === 0) return resampled;

    const firstTime = this.rawTrades[0].openTime.getTime();
    const lastTime = this.rawTrades[n - 1].openTime.getTime();
    ...
    for (let i = 0; i < n; i++) {
      ...
      const originalTrade = this.rawTrades[randomIndex];
